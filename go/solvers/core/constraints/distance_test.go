...
func TestDistanceEvaluator_PtPt_Jacobian(t *testing.T) {
	p1 := &schema.Entity{Id: "p1", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 1.0, Y: 2.0}}}
	p2 := &schema.Entity{Id: "p2", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 4.0, Y: 6.0}}}
	c := &schema.Constraint{
		Id: "c",
		ConstraintType: &schema.Constraint_Distance{
			Distance: &schema.DistanceConstraint{EntityA: "p1", EntityB: "p2", Value: 5.0},
		},
	}
	eval, err := constraints.NewEvaluator(c, map[gcstypes.EntityID]*schema.Entity{"p1": p1, "p2": p2})
	if err != nil {
	...
	paramIndices := map[gcstypes.EntityID]int{"p1": 0, "p2": 2}
	n := 4
	m := je.NumEquations()

	if m != 1 {
		t.Errorf("expected 1 equation for Pt-Pt, got %d", m)
	}

	for k := 0; k < 10; k++ {
		x := []float64{
			1.0 + (rng.Float64()-0.5)*0.5, 2.0 + (rng.Float64()-0.5)*0.5,
			4.0 + (rng.Float64()-0.5)*0.5, 6.0 + (rng.Float64()-0.5)*0.5,
		}
