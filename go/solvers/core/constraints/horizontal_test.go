...
func TestHorizontalEvaluator(t *testing.T) {
	p1a := &schema.Entity{Id: "p1a", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 0.0, Y: 0.0}}}
	p2a := &schema.Entity{Id: "p2a", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 2.0, Y: 0.1}}}
	
	l1 := &schema.Entity{Id: "l1", EntityType: &schema.Entity_Line{Line: &schema.LineEntity{P1Id: "p1a", P2Id: "p2a"}}}
	c := &schema.Constraint{
		Id: "c",
		ConstraintType: &schema.Constraint_Horizontal{
			Horizontal: &schema.HorizontalConstraint{LineId: "l1"},
		},
	}
	scenario := &schema.Sketch{Entities: []*schema.Entity{p1a, p2a, l1}, Constraints: []*schema.Constraint{c}}
	...
	for k := 0; k < 10; k++ {
		x := []float64{
			...
			2.0 + (rng.Float64()-0.5)*0.2, 0.1 + (rng.Float64()-0.5)*0.2,
		}
