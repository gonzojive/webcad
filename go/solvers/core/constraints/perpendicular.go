...
func (p *PerpendicularEvaluator) EvaluateJacobian(
	...
	if J != nil {
		invC := 1.0 / p.normC
		J.Set(rowOffset, idx1a, -dx2*invC)
		J.Set(rowOffset, idx1a+1, -dy2*invC)
		J.Set(rowOffset, idx2a, dx2*invC)
		J.Set(rowOffset, idx2a+1, dy2*invC)

		J.Set(rowOffset, idx1b, -dx1*invC)
		J.Set(rowOffset, idx1b+1, -dy1*invC)
		J.Set(rowOffset, idx2b, dx1*invC)
		J.Set(rowOffset, idx2b+1, dy1*invC)
	}
}
...
func (p *PerpendicularEvaluator) Evaluate(x []float64, grad []float64, paramIndices map[gcstypes.EntityID]int) float64 {
	...
	if grad != nil {
		...
		grad[idx1a] -= factor * dx2
		grad[idx1a+1] -= factor * dy2
		grad[idx2a] += factor * dx2
		grad[idx2a+1] += factor * dy2

		grad[idx1b] -= factor * dx1
		grad[idx1b+1] -= factor * dy1
		grad[idx2b] += factor * dx1
		grad[idx2b+1] += factor * dy1
	}
