...
func TestHorizontalDistanceEvaluator(t *testing.T) {
	p1 := &schema.Entity{Id: "p1", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 1.0, Y: 2.0}}}
	p2 := &schema.Entity{Id: "p2", EntityType: &schema.Entity_Point{Point: &schema.PointEntity{X: 4.0, Y: 3.0}}}
	
	c := &schema.Constraint{
		Id: "c",
		ConstraintType: &schema.Constraint_HorizontalDistance{
			HorizontalDistance: &schema.HorizontalDistanceConstraint{EntityA: "p1", EntityB: "p2", Value: 3.0},
		},
	}
	...
	for k := 0; k < 10; k++ {
		x := []float64{
			...
			4.0 + (rng.Float64()-0.5)*0.5, 3.0 + (rng.Float64()-0.5)*0.5,
		}
