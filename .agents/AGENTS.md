...
- **Gazelle build generator**: Use Gazelle to manage and generate `BUILD.bazel` files for Go packages. Run `bazel run //:gazelle` to automatically update or regenerate targets whenever Go source files, imports, or dependencies in `go.mod` change.
