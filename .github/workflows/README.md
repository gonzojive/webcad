...
1. Status checks are registered and updated dynamically under custom names (like `Check devtool tidy results in no diff` and `Bazel Build and Test`).
...
sequenceDiagram
    ...
    runner->>devtool: Run `./devtool github-ci register --name "Check devtool tidy results in no diff" --name "Bazel Build and Test"`
    activate devtool
...
```bash
./devtool github-ci register --name "Check devtool tidy results in no diff" --name "Bazel Build and Test" --sha "<commit-sha>"
```
