...
1. Status checks are registered and updated dynamically under custom names (like `Check Workspace Tidy` and `Bazel Build and Test`).
...
sequenceDiagram
    ...
    runner->>devtool: Run `./devtool github-ci register --name "Check Workspace Tidy" --name "Bazel Build and Test"`
    activate devtool
    devtool->>GH_API: POST /statuses/{sha} (pending: Check Workspace Tidy)
    devtool->>GH_API: POST /statuses/{sha} (pending: Bazel Build and Test)
    ...
    note over runner, devtool: Step 1: Check Workspace Tidy
    runner->>devtool: Run `./devtool github-ci run-check --name "Check Workspace Tidy" --preregistered -- <cmd>`
    activate devtool
    ...
    note over runner, devtool: Step 2: Bazel Build and Test
    runner->>devtool: Run `./devtool github-ci run-check --name "Bazel Build and Test" --preregistered -- <cmd>`
    activate devtool
    devtool->>GH_API: GET /statuses/{sha} (Verify check is pre-registered)
    devtool->>GH_API: POST /statuses/{sha} (State: "pending", Description: "Check is running...")
    devtool->>runner: Run subprocess: bazel build //... && bazel test //...
    runner-->>devtool: Subprocess exit code (0 = success)
    devtool->>GH_API: POST /statuses/{sha} (State: "success", Description: "Check completed successfully")
    deactivate devtool
...
```bash
./devtool github-ci register --name "Check Workspace Tidy" --name "Bazel Build and Test" --sha "<commit-sha>"
```

### 2. Execute a command and update status:
```bash
./devtool github-ci run-check --name "Check Workspace Tidy" --sha "<commit-sha>" --preregistered -- ./devtools/tidy && git diff --exit-code
```
