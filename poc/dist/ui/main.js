...
let hoveredConstraintId = null;
let currentTool = 'select';
// Temporary drawing states
let lineStartPointId = null;
let circleCenterPointId = null;
let tempLinePreview = null;
let tempCirclePreview = null;
// Snap State
...
async function initSolver() {
    try {
        ...
        if (statusText)
            statusText.innerText = 'Loading GCS...';
...
function findClosestPoint(x, y, tolerance = 12) {
    let closest = null;
...
function renderSidebar() {
    ...
    if (conContainer) {
        ...
        constraints.forEach(con => {
            ...
            switch (con.type) {
                case 'coincident':
                    desc = `Coincident: ${con.p1Id} & ${con.p2Id}`;
                    break;
                case 'distance':
                    desc = `Distance: ${con.p1Id} & ${con.p2Id} = ${con.value.toFixed(1)}`;
                    break;
                case 'vertical':
                    desc = `Vertical: ${con.lineId}`;
                    break;
                case 'horizontal':
                    desc = `Horizontal: ${con.lineId}`;
                    break;
                case 'parallel':
                    desc = `Parallel: ${con.line1Id} & ${con.line2Id}`;
                    break;
                case 'perpendicular':
                    desc = `Perpendicular: ${con.line1Id} & ${con.line2Id}`;
                    break;
            }
            item.innerHTML = `
                <div class="item-info">
    ...
    deleteConBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const id = btn.getAttribute('data-con-delete');
            if (id)
                deleteConstraint(id);
        });
    });
...
function isConstraintEntityHovered(entityId) {
...
function showInlineDistanceInput(p1, p2) {
...
function showValDialog(title, defaultValue, callback) {
