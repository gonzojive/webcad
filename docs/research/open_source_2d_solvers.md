...
8. **Establish UI-focused solver benchmarking**: To build a highly responsive sketching tool, WebCAD needs a dedicated GCS benchmarking suite aligned with user interface performance goals. The suite should simulate high-frequency mouse dragging events, measure solver latency (targeting consistent <16ms frame times), and quantify solve "discontinuity" (detecting and scoring sudden coordinate jumps, erratic geometric flipping, or divergence issues during interactive dragging).
