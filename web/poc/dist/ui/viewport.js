...
export class CanvasViewport {
    ...
    setEntityClickCallback(cb) {
        this.onEntityClick = cb;
    }
    setStageMouseDownCallback(cb) {
        this.onStageMouseDown = cb;
    }
    setStageMouseMoveCallback(cb) {
    ...
    updateSnapIndicator(x, y, visible) {
    ...
    redrawAll() {
        ...
        const isConstraintEntityHovered = (entityId) => {
        ...
        this.model.getLines().forEach(l => {
            ...
            lineShape.on('click', (e) => {
                if (this.model.getTool() === 'select') {
                    e.cancelBubble = true;
                    if (this.onEntityClick)
                        this.onEntityClick(l.id, e);
                }
            });
    ...
    updateEntityVisuals() {
        ...
        // Update Lines
        this.model.getLines().forEach(l => {
            const lineShape = this.mainLayer.findOne('#' + l.id);
            if (!lineShape)
                return;
            const isSelected = selectedEntityIds.includes(l.id);
            const isHovered = hoveredEntityId === l.id || isConstraintEntityHovered(l.id);
        ...
        // Update Circles
        this.model.getCircles().forEach(c => {
            const circleShape = this.mainLayer.findOne('#' + c.id);
            if (!circleShape)
                return;
            const isSelected = selectedEntityIds.includes(c.id);
            const isHovered = hoveredEntityId === c.id || isConstraintEntityHovered(c.id);
        ...
        // Update Points
        this.model.getPoints().forEach(p => {
            if (p.id === this.draggedPointId)
                return;
            const pointGroup = this.mainLayer.findOne('#' + p.id);
            if (!pointGroup)
                return;
            const isSelected = selectedEntityIds.includes(p.id);
            const isHovered = hoveredEntityId === p.id || isConstraintEntityHovered(p.id);
    ...
    updateLineVisualPosition(lineId, x1, y1, x2, y2) {
        const lineShape = this.mainLayer?.findOne('#' + lineId);
        if (lineShape) {
            lineShape.points([x1, y1, x2, y2]);
        }
    }
    updateCircleVisualPosition(circleId, cx, cy, r) {
    ...
    updatePointVisualPosition(pointId, x, y) {
